...
async function handleNovProxy(bot, query) {
  ...
  if (data.startsWith("nov_proxy_")) {
    ...
    const pkg = [...novProxyDiscountPackages, ...novProxyRegularPackages].find(
      (item) => item.callback === data
    );

    await showPaymentMethods(bot, chatId, {
      productKey: "nov_proxy",
      itemKey: data,
      name: "Nov Proxy",
      package: pkg?.name || data.replace("nov_proxy_", "").toUpperCase(),
      price: pkg?.price || "Contact Support",
      back: "novproxy"
    });
